...
function GoogleSignInButton({ onSuccess, disabled }) {
  ...
  useEffect(() => {
    ...
    // Small delay to ensure the container is in DOM and has dimensions
  ...
  return (
    <div className={`w-full ${disabled ? 'opacity-50 pointer-events-none' : ''}`}>
      {/* Google's rendered button goes here */}
      <div
        ref={btnContainerRef}
        className="flex items-center justify-center"
        style={{ minHeight: '44px' }}
      />
      {/* Fallback if GSI render fails after 3s */}
      {!gsiReady && (
        <button
          type="button"
          disabled={disabled}
          className="w-full px-5 py-2.5 rounded-xl bg-white hover:bg-gray-50 border border-gray-200 text-slate-700 text-sm font-medium transition-all flex items-center justify-center gap-3"
        >
