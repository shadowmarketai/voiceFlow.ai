...
import { Search, ExternalLink, Check, ArrowUpRight, X, Loader2, Settings, Unplug, Link2 } from 'lucide-react'
...
const categoryColors = {
  ...
  'AI / Voice': { bg: 'bg-violet-50', text: 'text-violet-600', ring: 'from-violet-500 to-violet-600' },
...
const INITIAL_INTEGRATIONS = [
  // CRM
  { id: 'hubspot', name: 'HubSpot', category: 'CRM', description: 'Sync contacts, deals, and call logs with HubSpot CRM.', connected: false, logo: 'HS', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'pat-xxx' }] },
  { id: 'salesforce', name: 'Salesforce', category: 'CRM', description: 'Push call outcomes and lead data to Salesforce automatically.', connected: false, logo: 'SF', configFields: [{ key: 'client_id', label: 'Client ID', placeholder: 'xxx' }, { key: 'client_secret', label: 'Client Secret', placeholder: 'xxx' }] },
  { id: 'zoho', name: 'Zoho CRM', category: 'CRM', description: 'Bi-directional sync with Zoho CRM for contacts and activities.', connected: false, logo: 'ZO', configFields: [{ key: 'client_id', label: 'Client ID', placeholder: '1000.xxx' }, { key: 'client_secret', label: 'Client Secret', placeholder: 'xxx' }] },
  { id: 'leadsquared', name: 'LeadSquared', category: 'CRM', description: 'Indian CRM for lead capture and nurturing workflows.', connected: false, logo: 'LS', configFields: [{ key: 'access_key', label: 'Access Key', placeholder: 'xxx' }, { key: 'secret_key', label: 'Secret Key', placeholder: 'xxx' }] },
  { id: 'freshsales', name: 'Freshsales', category: 'CRM', description: 'AI-powered CRM by Freshworks — built for Indian businesses.', connected: false, logo: 'FS', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'xxx' }, { key: 'domain', label: 'Domain', placeholder: 'yourcompany.freshsales.io' }] },
  // AI / Voice
  { id: 'groq', name: 'Groq', category: 'AI / Voice', description: 'Ultra-fast LLM inference (~100ms) with Llama 3 for voice agents.', connected: false, logo: 'GQ', badge: 'Fastest', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'gsk_xxx' }] },
  { id: 'anthropic', name: 'Anthropic Claude', category: 'AI / Voice', description: 'High-quality AI conversations with Claude for complex queries.', connected: false, logo: 'CL', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'sk-ant-xxx' }] },
  { id: 'openai', name: 'OpenAI', category: 'AI / Voice', description: 'GPT-4 for AI responses + Whisper for speech-to-text.', connected: false, logo: 'OA', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'sk-xxx' }] },
  { id: 'deepgram', name: 'Deepgram', category: 'AI / Voice', description: 'Real-time speech-to-text with streaming WebSocket support.', connected: false, logo: 'DG', badge: 'STT', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'xxx' }] },
  { id: 'elevenlabs', name: 'ElevenLabs', category: 'AI / Voice', description: 'Best-in-class voice cloning + TTS for lifelike AI voices.', connected: false, logo: 'EL', badge: 'Cloning', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'sk_xxx' }] },
  { id: 'sarvam', name: 'Sarvam AI', category: 'AI / Voice', description: 'Indian-built: STT + TTS + translation for all Indic languages.', connected: false, logo: 'SV', badge: 'Indic', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'xxx' }] },
  // Automation
  { id: 'zapier', name: 'Zapier', category: 'Automation', description: 'Connect VoiceFlow AI with 5,000+ apps via automated workflows.', connected: false, logo: 'ZP', configFields: [{ key: 'webhook_url', label: 'Webhook URL', placeholder: 'https://hooks.zapier.com/xxx' }] },
  { id: 'n8n', name: 'n8n', category: 'Automation', description: 'Self-hosted workflow automation with full data control.', connected: false, logo: 'N8', configFields: [{ key: 'webhook_url', label: 'Webhook URL', placeholder: 'https://n8n.yourserver.com/webhook/xxx' }] },
  { id: 'make', name: 'Make', category: 'Automation', description: 'Visual automation platform for complex multi-step workflows.', connected: false, logo: 'MK', configFields: [{ key: 'webhook_url', label: 'Webhook URL', placeholder: 'https://hook.make.com/xxx' }] },
  // Calendar
  { id: 'google-calendar', name: 'Google Calendar', category: 'Calendar', description: 'Schedule appointments directly from voice conversations.', connected: false, logo: 'GC', configFields: [{ key: 'client_id', label: 'OAuth Client ID', placeholder: 'xxx.apps.googleusercontent.com' }, { key: 'client_secret', label: 'Client Secret', placeholder: 'GOCSPX-xxx' }] },
  { id: 'calcom', name: 'Cal.com', category: 'Calendar', description: 'Open-source scheduling for booking meetings from calls.', connected: false, logo: 'CC', configFields: [{ key: 'api_key', label: 'API Key', placeholder: 'cal_xxx' }] },
]
...
export default function Integrations() {
  ...
  const handleTestConnection = (integrationId) => {
    ...
    toast.promise(
      ...
      {
        loading: `Testing ${integration?.name} connection...`,
        success: `${integration?.name} connection is healthy`,
  ...
  const openConnectModal = (integration) => {
    setConfigValues({})
    setConnectModal(integration.id)
  }
    ...
    <div className="space-y-6">
      ...
      >
        {filtered.map((integration) => {
          ...
          return (
            <motion.div
              key={integration.id} variants={item}
      ...
      <AnimatePresence>
        {connectModal && (() => {
          const integration = integrations.find(i => i.id === connectModal)
            ...
            >
              ...
              >
                <div className="flex items-center gap-3 mb-5">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${colors.ring} flex items-center justify-center text-sm font-bold text-white`}>
                    {integration.logo}
                  </div>
                ...
                >
                  {connecting ? <><Loader2 className="w-4 h-4 animate-spin" /> Connecting...</> : <><Link2 className="w-4 h-4" /> Connect {integration.name}</>}
                </button>
              </motion.div>
            </motion.div>
          )
      ...
      <AnimatePresence>
        {manageModal && (() => {
          const integration = integrations.find(i => i.id === manageModal)
          if (!integration) return null
          return (
            <motion.div
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            >
              ...
              >
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-lg font-semibold text-gray-900">Manage {integration.name}</h3>
                ...
                <div className="space-y-3 mb-5">
                  ...
                  {integration.config && Object.entries(integration.config).map(([key, val]) => (
